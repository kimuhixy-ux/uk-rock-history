...
BASE = "https://kimuhixy.com/uk-rock-history"
...
def schema(row: dict, genre: str, artist_type: str, english: bool) -> str:
    ...
    graph = [
        {"@type": "WebSite", "@id": f"{BASE}/#website", "url": f"{BASE}/", "name": "UK Rock History", "inLanguage": ["ja", "en"]},
...
def detail_context(row: dict, genre: str, related: list[int], rows: list[dict], artist_types: dict[str, str], english: bool) -> dict[str, str]:
    ...
    meta = f"{title} by {artist}, released in {data['year']}. View genre facts in the UK Rock History essential album guide." if english else f"{artist}『{title}』（{data['year']}年）のジャンルをUK Rock History名盤ガイドの事実情報で確認できます。"
    return {
        "lang": "en" if english else "ja", "title": esc(title), "artist": esc(artist), "genre": esc(genre),
        "page_title": esc(f"{title} — {artist} ({data['year']}) | UK Rock History"), "meta_description": esc(meta[:155]),
...
def index_context(rows: list[dict], labels: dict, english: bool) -> dict[str, str]:
    ...
    title = "Essential UK Rock Album Index" if english else "英国ロック名盤索引"
    canonical = f"{BASE}/{prefix}items/"
    graph = [{"@type": "WebSite", "@id": f"{BASE}/#website", "url": f"{BASE}/", "name": "UK Rock History", "inLanguage": ["ja", "en"]}, {"@type": "CollectionPage", "url": canonical, "name": title, "inLanguage": "en" if english else "ja", "isPartOf": {"@id": f"{BASE}/#website"}}]
    return {
        "lang": "en" if english else "ja", "title": title, "page_title": f"{title} | UK Rock History",
        "meta_description": f"Browse {len(rows)} essential UK rock albums by genre, with artist and release-year facts." if english else f"英国ロックの精選名盤{len(rows)}枚をジャンル別に探せる索引です。アーティストと発表年を確認できます。",
