...
export const S = {
  ...
  timelineLead: en
    ? "Explore British rock artists by the decade they began their career, from the 1960s to today."
    : "1960年代から現在まで、活動開始年ごとにイギリスのロックアーティストを辿れます。",
  seeMoreArtists: (n) => (en ? `See more artists from this era (${n}) →` : `この年代のアーティストをもっと見る(${n}組)→`),

  // ===== artists.js =====
  artistsTitle: en ? "Artists" : "アーティスト一覧",
  artistsLead: (n) => (en ? `Search and filter all ${n} British rock artists.` : `全${n}組のイギリスのロックアーティストを検索・絞り込みできます。`),
  ...
  genresLead: en
    ? "How the major genres in British rock branched from one another. Tap a node to see artists in that genre."
    : "イギリスのロックシーンにおける主なジャンルの派生関係です。ノードをタップするとそのジャンルのアーティスト一覧に移動します。",
  genresSvgAriaLabel: en ? "Genre family tree diagram" : "ジャンル系統図",

  // ===== glossary.js =====
  glossaryTitle: en ? "Glossary" : "用語集",
  glossaryLead: en
    ? "Common terms used in the British rock scene."
    : "イギリスのロックシーンでよく使われる用語をまとめました。",
  ...
  statsLead: (n) => (en ? `Trends in British rock, based on all ${n} artists in the dataset.` : `収集したデータ(全${n}組)から見る、イギリスロックの傾向です。`),
